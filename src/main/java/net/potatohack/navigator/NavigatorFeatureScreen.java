 ...
 */
package net.potatohack.navigator;
...
import net.potatohack.clickgui.Component;
...
import net.potatohack.keybinds.Keybind;
...
public final class NavigatorFeatureScreen extends NavigatorScreen
{
		...
		if(hasPrimaryAction)
		{
			primaryButton =
				ButtonWidget.builder(Text.literal(primaryAction), b -> {
					...
					PotatoHack.INSTANCE.getNavigator()
		...
		if(!possibleKeybinds.isEmpty())
		{
			...
			ButtonData addKeybindButton =
					...
					area.y + getStringHeight(text) - 7, 12, 8, "+", 0x00ff00)
				{
					...
					{
						// add keybind
						PotatoHack.MC.setScreen(new NavigatorNewKeybindScreen(
			...
			for(Keybind keybind : PotatoHack.INSTANCE.getKeybinds()
				.getAllKeybinds())
			{
	...
	{
		// popups
		if(PotatoHack.INSTANCE.getGui().handleNavigatorPopupClick(x, y,
			button))
			return;
		...
		// component settings
		PotatoHack.INSTANCE.getGui().handleNavigatorMouseClick(
	...
	{
		PotatoHack.INSTANCE.getGui().handleMouseRelease(x, y, button);
	}
	...
	{
		window.close();
		PotatoHack.INSTANCE.getGui().handleMouseClick(Integer.MIN_VALUE,
			Integer.MIN_VALUE, 0);
	}
