...
import net.potatohack.util.ListWidget;
...
public final class KeybindManagerScreen extends Screen
{
	...
	public void init()
	{
		...
		addDrawableChild(ButtonWidget.builder(Text.literal("Reset Keybinds"),
			b -> client.setScreen(new ConfirmScreen(confirmed -> {
				if(confirmed)
					PotatoHack.INSTANCE.getKeybinds()
	...
	private void edit()
	{
		Keybind keybind = PotatoHack.INSTANCE.getKeybinds().getAllKeybinds()
	...
	private void remove()
	{
		Keybind keybind1 = PotatoHack.INSTANCE.getKeybinds().getAllKeybinds()
			.get(listGui.selected);
		PotatoHack.INSTANCE.getKeybinds().remove(keybind1.getKey());
	}
	...
	private static final class ListGui extends ListWidget
	{
		...
		{
			return PotatoHack.INSTANCE.getKeybinds().getAllKeybinds().size();
		}
		...
		{
			Keybind keybind =
				PotatoHack.INSTANCE.getKeybinds().getAllKeybinds().get(index);
