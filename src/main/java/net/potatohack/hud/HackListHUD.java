...
import net.potatohack.other_features.HackListOtf;
import net.potatohack.other_features.HackListOtf.Mode;
import net.potatohack.other_features.HackListOtf.Position;

public final class HackListHUD implements UpdateListener
{
	private final ArrayList<HackListEntry> activeHax = new ArrayList<>();
	private final HackListOtf otf = PotatoHack.INSTANCE.getOtfs().hackListOtf;
	private int posY;
	private int textColor;
	
	public HackListHUD()
	{
		PotatoHack.INSTANCE.getEventManager().add(UpdateListener.class, this);
	}
	...
	public void render(DrawContext context, float partialTicks)
	{
		...
		if(otf.getPosition() == Position.LEFT
			&& PotatoHack.INSTANCE.getOtfs().wurstLogoOtf.isVisible())
			posY = 22;
		else
			posY = 2;
		...
		{
			float[] acColor = PotatoHack.INSTANCE.getGui().getAcColor();
	...
	private void drawString(DrawContext context, String s)
	{
		...
		{
			int screenWidth = PotatoHack.MC.getWindow().getScaledWidth();
