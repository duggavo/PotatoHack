...
import net.potatohack.hacks.TooManyHaxHack;
import net.potatohack.other_features.TabGuiOtf;
...
public final class TabGui implements KeyPressListener
{
	...
	private final TabGuiOtf tabGuiOtf =
		PotatoHack.INSTANCE.getOtfs().tabGuiOtf;
	...
	public TabGui()
	{
		PotatoHack.INSTANCE.getEventManager().add(KeyPressListener.class,
			this);
		...
		features.addAll(PotatoHack.INSTANCE.getHax().getAllHax());
		features.addAll(PotatoHack.INSTANCE.getCmds().getAllCmds());
		features.addAll(PotatoHack.INSTANCE.getOtfs().getAllOtfs());
	...
	private void updateSize()
	{
		...
		for(Tab tab : tabs)
		{
			int tabWidth = PotatoHack.MC.textRenderer.getWidth(tab.name) + 10;
			if(tabWidth > width)
				width = tabWidth;
		}
		height = tabs.size() * 10;
	...
	public void render(DrawContext context, float partialTicks)
	{
		...
		ClickGui gui = PotatoHack.INSTANCE.getGui();
		...
		matrixStack.push();
		Window sr = PotatoHack.MC.getWindow();
		...
		for(int i = 0; i < tabs.size(); i++)
		{
			...
			context.drawText(PotatoHack.MC.textRenderer, tabName, 2, textY,
				txtColor, false);
			textY += 10;
		}
		GL11.glEnable(GL11.GL_BLEND);
		...
		if(tabOpened)
		{
			...
			for(int i = 0; i < tab.features.size(); i++)
			{
				...
				context.drawText(PotatoHack.MC.textRenderer, fName, 2,
	...
	private static final class Tab
	{
		...
		public void updateSize()
		{
			...
			{
				int fWidth =
					PotatoHack.MC.textRenderer.getWidth(feature.getName())
						+ 10;
				if(fWidth > width)
					width = fWidth;
			}
		...
		private void onEnter()
		{
			...
			TooManyHaxHack tooManyHax =
				PotatoHack.INSTANCE.getHax().tooManyHaxHack;
