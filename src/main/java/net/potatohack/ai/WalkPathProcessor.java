...
public class WalkPathProcessor extends PathProcessor
{
	...
	public void process()
	{
		...
		if(PotatoHack.MC.player.isOnGround())
			pos = BlockPos.ofFloored(PotatoHack.MC.player.getX(),
				PotatoHack.MC.player.getY() + 0.5,
				PotatoHack.MC.player.getZ());
		else
			pos = BlockPos.ofFloored(PotatoHack.MC.player.getPos());
		...
		lockControls();
		PotatoHack.MC.player.getAbilities().flying = false;
		...
		if(WURST.getHax().jesusHack.isEnabled())
		{
			...
			if(PotatoHack.MC.player.getY() < nextPos.getY()
				&& (PotatoHack.MC.player.isTouchingWater()
					|| PotatoHack.MC.player.isInLava()))
				return;
			
			// manually swim down if using Jesus
			if(PotatoHack.MC.player.getY() - nextPos.getY() > 0.5
				&& (PotatoHack.MC.player.isTouchingWater()
					|| PotatoHack.MC.player.isInLava()
