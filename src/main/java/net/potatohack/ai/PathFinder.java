...
{
	private final PotatoHack wurst = PotatoHack.INSTANCE;
	
	private final boolean invulnerable =
		PotatoHack.MC.player.getAbilities().creativeMode;
	private final boolean creativeFlying =
		PotatoHack.MC.player.getAbilities().flying;
	...
	{
		if(PotatoHack.MC.player.isOnGround())
			start = new PathPos(BlockPos.ofFloored(PotatoHack.MC.player.getX(),
				PotatoHack.MC.player.getY() + 0.5,
				PotatoHack.MC.player.getZ()));
		else
			start =
				new PathPos(BlockPos.ofFloored(PotatoHack.MC.player.getPos()));
		this.goal = goal;
	...
	private ArrayList<PathPos> getNeighbors(PathPos pos)
	{
		...
		// up
		if(pos.getY() < PotatoHack.MC.world.getTopY() && canGoThrough(up.up())
		...
		// down
		if(pos.getY() > PotatoHack.MC.world.getBottomY() && canGoThrough(down)
	...
	private boolean canGoThrough(BlockPos pos)
	{
		...
		if(!PotatoHack.MC.world.isChunkLoaded(pos))
			return false;
	...
	public boolean isPathStillValid(int index)
	{
		...
		if(invulnerable != PotatoHack.MC.player.getAbilities().creativeMode
