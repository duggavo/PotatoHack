...
import net.potatohack.RotationFaker;
...
public enum RotationUtils
{
	...
	{
		RotationFaker rotationFaker = PotatoHack.INSTANCE.getRotationFaker();
	...
	public static double getAngleToLastReportedLookVec(Vec3d vec)
	{
		...
		IClientPlayerEntity player = PotatoHack.IMC.getPlayer();
	...
	{
		Rotation needed = getNeededRotations(vec);
		return MathHelper.wrapDegrees(PotatoHack.MC.player.getYaw())
			- needed.yaw;
	}
	...
	{
		Entity player = PotatoHack.MC.player;
