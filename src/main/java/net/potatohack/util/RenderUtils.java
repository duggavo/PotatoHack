...
public enum RenderUtils
{
	...
	public static void scissorBox(int startX, int startY, int endX, int endY)
	{
		...
		int bottomY = PotatoHack.MC.currentScreen.height - endY;
		double factor = PotatoHack.MC.getWindow().getScaleFactor();
	...
	public static void applyCameraRotationOnly()
	{
		...
		// Camera camera =
		// PotatoHack.MC.getBlockEntityRenderDispatcher().camera;
	...
	{
		Camera camera = PotatoHack.MC.getBlockEntityRenderDispatcher().camera;
		if(camera == null)
			return Vec3d.ZERO;
